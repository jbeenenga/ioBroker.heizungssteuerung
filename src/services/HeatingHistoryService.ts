...
export class HeatingHistoryService {
	...
	private analyzeCycle(room: string, measurements: TemperatureMeasurement[], startTime: number): HeatingCycle | null {
		...
		if (heatingMeasurements.length === 0) {
			return null;
		}
		...
		const endTemp =
			engineStopIndex > 0
				? measurements[engineStopIndex - 1].temperature
				: measurements[measurements.length - 1].temperature;
		...
		if (heatingMeasurements.length >= 2) {
			const heatingDuration =
				(heatingMeasurements[heatingMeasurements.length - 1].timestamp - heatingMeasurements[0].timestamp) /
				1000 /
				3600; // hours
			const tempChange =
				heatingMeasurements[heatingMeasurements.length - 1].temperature - heatingMeasurements[0].temperature;
		...
		if (coolingMeasurements.length >= 2) {
			const cooldownDuration =
				(coolingMeasurements[coolingMeasurements.length - 1].timestamp - coolingMeasurements[0].timestamp) /
				1000 /
				3600; // hours
			const tempChange =
				coolingMeasurements[coolingMeasurements.length - 1].temperature - coolingMeasurements[0].temperature;
			cooldownRate = cooldownDuration > 0 ? tempChange / cooldownDuration : 0;
		}

		// Average outside temperature
		const outsideTemps = measurements
			.filter(m => m.outsideTemperature !== undefined)
			.map(m => m.outsideTemperature!);
		const avgOutsideTemp =
			outsideTemps.length > 0 ? outsideTemps.reduce((a, b) => a + b, 0) / outsideTemps.length : undefined;
	...
	private updateRoomProfile(room: string): void {
		...
		const avgCooldownRate = Math.abs(
			recentCycles.reduce((sum, c) => sum + c.cooldownRate, 0) / recentCycles.length,
		);
		...
		for (const cycle of cycles) {
			...
			for (let i = 0; i < heatingMeasurements.length - 1; i++) {
				...
				if (recentMeasurements.length >= 2) {
					const duration =
						(recentMeasurements[recentMeasurements.length - 1].timestamp -
							recentMeasurements[0].timestamp) /
						1000 /
						3600;
					const tempChange =
						recentMeasurements[recentMeasurements.length - 1].temperature -
						recentMeasurements[0].temperature;
