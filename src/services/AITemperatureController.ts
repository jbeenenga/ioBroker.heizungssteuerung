...
export class AITemperatureController extends TemperatureController {
    ...
    private recentTemperatures: Map<string, Array<{ temp: number; time: number }>> = new Map();
    private lastRetrainCheck = Date.now();
    private predictionCache: Map<string, { prediction: any; timestamp: number }> = new Map();
    ...
    private initializeAI(): void {
        ...
        this.historyService = new HeatingHistoryService(
    ...
    public shouldActivateEngine(
        ...
        if (this.historyService && context) {
            this.recordMeasurement(context.room, currentTemp, targetTemp, context.lastEngineState, humidity, context.outsideTemp);
        }
        ...
        this.updatePredictionCache(context.room, currentTemp, targetTemp, context.heatingDuration, context.recentHeatingRate, profile, context.outsideTemp);
    ...
    private updatePredictionCache(
        ...
        this.aiPredictor!.predict(room, currentTemp, targetTemp, heatingDuration, recentHeatingRate, profile, outsideTemp)
            .then(prediction => {
                if (prediction) {
    ...
    private shouldActivateEngineAI(
        ...
        if (this.aiConfig.isHeatingMode === 1 && humidity !== undefined && humidity > this.aiConfig.stopCoolingIfHumIsHigherThan) {
            return false;
        }
    ...
    private recordMeasurement(
        ...
        this.historyService.recordMeasurement(
            room,
            currentTemp,
            targetTemp,
            engineState,
        ...
        this.recentTemperatures.set(room, recent.filter(r => r.time > cutoff));
    }
    ...
    public getAIStatus(): {
        ...
        if (!this.historyService) return status;
