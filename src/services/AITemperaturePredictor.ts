...
export class AITemperaturePredictor {
    ...
    private normalizeInputs(dataPoints: TrainingDataPoint[]): {
        ...
        const normalizedInputs = inputs.map(input =>
            input.map((val, i) => (val - mean[i]) / std[i])
        );
     ...
     * Save model to disk
     */
    private async saveModel(room: string, model: tf.LayersModel, stats: { mean: number[]; std: number[] }): Promise<void> {
        try {
     ...
     * Get model info
     */
    public getModelInfo(room: string): { ready: boolean; training: boolean; lastTrained?: number } {
        return {
