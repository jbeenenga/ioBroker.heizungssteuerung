...
class Heizungssteuerung extends utils.Adapter {
  ...
  async onReady(): Promise<void> {
    ...
    this.tempSensorMap = await this.buildFunctionToRoomMap(
      "enum.functions.temperature",
      "Temperature",
    );
    this.humSensorMap = await this.buildFunctionToRoomMap(
      "enum.functions.humidity",
      "Humidity",
    );
    this.engineMap = await this.buildFunctionToRoomMap(
      "enum.functions.engine",
      "Engine",
    );
    this.log.debug(
      "tempSensorMap created: " + JSON.stringify(this.tempSensorMap),
    );
    this.log.debug(
      "humSensorMap created: " + JSON.stringify(this.humSensorMap),
    );
    ...
    this.interval = this.setInterval(
      ...
      this.config.updateIntervall * 1000,
    );
  }
  ...
  private async check(): Promise<void> {
    ...
    const boostedRooms = await this.buildSpecialRoomsList(
      "boost",
      this.config.boostIntervall,
    );
    const pausedRooms = await this.buildSpecialRoomsList(
      "pause",
      this.config.pauseIntervall,
    );
    ...
    if (pauseAll != undefined && pauseAll.val == true) {
      if (
        pauseAll.ts >
        new Date().getTime() - this.config.pauseIntervall * 60000
      ) {
        this.log.info(
          "State pauseAll is active so all engines will be deactivated",
        );
    ...
    const absenceActive =
      absenceUntil == null ||
      absenceUntil.val == null ||
      absenceUntil.val > nowAsDate;
    ...
    if (boostAll != undefined && boostAll.val == true) {
      if (
        boostAll.ts >
        new Date().getTime() - this.config.boostIntervall * 60000
      ) {
        this.log.info(
          "State boostAll is active so all engines will be deactivated",
        );
    ...
    this.log.debug(
      "Temperatures will be set like: " + JSON.stringify(roomTempMap),
    );

    for (let i = 0; i < this.roomNames.length; i++) {
      this.setTemperatureForRoom(
        this.roomNames[i],
  ...
  private fillRommTemperatures(
    ...
    if (pausedRooms.includes(currentRoom)) {
      ...
      roomTempMap.set(currentRoom, {
        temp: this.getPauseTemperature(),
        until: "pause",
      });
      return;
    }
    ...
    if (absenceActive) {
      this.log.debug(
        "absence is active, so " +
          currentRoom +
          " will be set to default temperature.",
      );
      return;
    }
    ...
    this.log.debug(
      "found following periods for room " +
        currentRoom +
        ": " +
        JSON.stringify(periodsForRoom),
    );
    periodsForRoom.forEach((period) => {
      if (
        period.from > now &&
        period.from < roomTempMap.get(currentRoom)!.until
      ) {
        this.log.debug(
          "targetUntil for room " +
            currentRoom +
            " will be set to " +
            period.from,
        );
      ...
      if (
        roomTempMap.get(currentRoom)!.until > now &&
        roomTempMap.get(currentRoom)!.until != "24:00"
      ) {
        return;
      }
      if (
        (this.config.isHeatingMode == 0) == period.heating &&
        this.isCurrentPeriod(period, now)
      ) {
  ...
  private isPeriodValid(period: Period, autoCorrect: boolean): boolean {
    if (
      ...
      if (this.isPeriodValid(period, false)) {
        this.log.warn(
          "The given period is not valid and will be ignored: " +
  ...
  async buildDefaultRoomTempMap(now: string): Promise<Map<string, TempTarget>> {
    ...
    for (let i = 0; i < this.roomNames.length; i++) {
      ...
      const targetTemperatureFromState = await this.getStateAsync(
        "Temperatures." + room + ".target",
      );
      const targetTemperatureUntilFromState = await this.getStateAsync(
        "Temperatures." + room + ".targetUntil",
      );
      if (
        targetTemperatureFromState == undefined ||
        targetTemperatureUntilFromState == undefined
      ) {
        ...
        this.log.debug(
          "Target until from state is " +
            JSON.stringify(targetTemperatureUntilFromState),
        );
        if (
   ...
   */
  private async buildFunctionToRoomMap(
    functionId: string,
    functionName: string,
    ...
    for (let i = 0; i < funcTemp.common["members"].length; i++) {
      for (let j = 0; j < this.roomNames.length; j++) {
        for (
          let k = 0;
          k <
          this.rooms.result["enum.rooms." + this.roomNames[j]]["common"][
            "members"
          ].length;
          k++
        ) {
          if (
            this.rooms.result["enum.rooms." + this.roomNames[j]]["common"][
              "members"
            ][k] == funcTemp["common"]["members"][i]
          ) {
            functionToRoomMap.set(
              this.roomNames[j],
  ...
  async setTemperatureForRoom(
    ...
    const tempState = await this.getForeignStateAsync(
      this.tempSensorMap.get(room)!,
    );
    ...
    const humidity = await this.getForeignStateAsync(
      this.humSensorMap.get(room)!,
    );
    ...
    if (this.config.isHeatingMode == 0) {
      ...
      if (
        ...
        this.log.info(
          "Deactivate engine for " + room + " because humidity maximum reached",
        );
        this.setForeignStateAsync(this.engineMap.get(room)!, false);
        return;
      }
   ...
   */
  private async buildSpecialRoomsList(
    actionName: string,
    validIntervall: number,
    ...
    for (let i = 0; i < this.roomNames.length; i++) {
      const boost = await this.getStateAsync(
        "Actions." + this.roomNames[i] + "." + actionName,
      );
      if (boost != undefined && boost.val == true) {
        if (boost.ts > boostValidUntil) {
          ...
          this.setState(
            "Temperatures." + this.roomNames[i] + ".targetUntil",
            "00:00",
            true,
          );
        }
  ...
  writeTemperaturesIntoState(
    ...
    if (humidity != undefined && humidity.val != undefined) {
      this.setStateAsync(
        "Temperatures." + room + ".currentHumidity",
        Number(humidity.val),
        true,
      );
    }
